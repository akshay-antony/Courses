...
#optimizer = optim.Adam(policy_net.parameters(), lr=0.09)
...
def optimize_model():
    ...
    ##### ddqn uses the policy net to take the action instead of argmax on target
    future_state_q_values = torch.zeros(BATCH_SIZE, device=device)

    ####
    action_next_state_policy_net = policy_net(non_final_states).argmax(1) #.detach() #.reshape(-1, 1)   
    #### find q value target by using the action from previous step
    future_state_q_values[non_final_mask] = target_net(non_final_states) \
                                            [torch.arange(action_next_state_policy_net.shape[0]), 
                                                          action_next_state_policy_net]  
...
for i_episode in range(num_episodes):
    ...
    torch.save(policy_net.state_dict(), "q2_ddqn_policy_net.pth")
    torch.save(target_net.state_dict(), "q2_ddqn_target_net.pth")
# plot time duration
plt.figure()
