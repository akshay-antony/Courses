...
if __name__ == "__main__":
	...
	edge = cv2.GaussianBlur(input_image,(3,3), 1)
	sobelx = cv2.Sobel(edge,cv2.CV_64F,1,0,ksize=5)
	sobely = cv2.Sobel(edge,cv2.CV_64F,0,1,ksize=5)

	sob = np.uint8(np.sqrt(sobelx**2 + sobely**2))
	cv2.imshow("s", sobelx)
	cv2.waitKey(0)
